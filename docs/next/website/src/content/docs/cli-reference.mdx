...
`workspace rename` with an empty label clears the manual name, so the space follows its agent session label (or the automatic directory label) again.
...
`report-agent-session` updates native session identity without reporting lifecycle state. An optional `--session-name TEXT` names the reporting pane's space after the agent session (empty text clears it; omitted keeps it). `release-agent` ends that source's lifecycle authority when its agent process exits.
