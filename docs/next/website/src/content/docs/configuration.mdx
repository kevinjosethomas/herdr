...
`ui.equal_pane_splits = true` (the default) rebuilds a tab as one equal strip when a pane splits without an explicit ratio: `prefix+v` gives every pane in the tab the same width, `prefix+-` the same height, whatever the previous nested layout was. Explicit `pane split --ratio` values keep exact placement. Set it to `false` to restore classic nested half splits.
