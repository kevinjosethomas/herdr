...
`ui.equal_pane_splits = true` (the default) keeps every pane in a tab evenly divided: a pane split without an explicit ratio rebuilds the tab as one equal strip (`prefix+v` gives every pane the same width, `prefix+-` the same height), and closing a pane re-evens the rest in the same orientation. Explicit `pane split --ratio` values and pane moves keep exact placement. Set it to `false` to restore classic nested half splits.
...
Set `enabled = false` under `[ui.sidebar.agents]` to hide the agents section entirely:
...
The spaces section then fills the expanded sidebar height, with no agents header, rows, empty state, or section divider. The agents rail is hidden in the collapsed sidebar and the mobile switcher as well.
...
- `index` — 1-based position of the space in the sidebar, matching number-key switching.
- `workspace` — workspace name.
